...
const CMDS = [
  ...
  new SlashCommandBuilder().setName('birthday').setDescription('Birthday system.')
    ...
    .addSubcommand(s => s.setName('channel').setDescription('View or set the birthday announcement channel. (Admin)')
      .addChannelOption(o => o.setName('channel').setDescription('New announcement channel (leave blank to view current)')))
    .addSubcommand(s => s.setName('role').setDescription('View or set the birthday role. (Admin)')
      .addRoleOption(o => o.setName('role').setDescription('New birthday role (leave blank to view current)')))
...
async function handleHelp(i) {
    ...
    .addFields(
      {
        ...
        value: [
          ...
          '`/birthday view [user]` — View a birthday (yours or another member\'s)',
          '`/birthday remove` — Remove your birthday from the records',
          '`/birthday list` — See all birthdays in this server',
          '`/birthday test [user]` — Preview how the birthday announcement will look',
        ].join('\n'),
      },
      {
        name: '🎉 AFK',
        value: [
          '`/afk [message]` — Go AFK with an optional message',
          '`/return` — Come back and clear your AFK status',
          '*Bot auto-notifies anyone who pings you while AFK.*',
        ].join('\n'),
      },
      {
        name: '🔍 Info',
        value: [
          '`/userinfo [user]` — ID, join date, roles, warn & case count, watch status',
          '`/roleinfo <role>` — Colour, member count, key permissions',
        ].join('\n'),
      },
      {
        name: '✨ Aesthetic',
        value: [
          '`/bless <user>` — Bestow the Queen\'s favour',
          '`/curse <user>` — Lay a shadow upon a user',
        ].join('\n'),
      },
    )
    ...
    .addFields(
      {
        ...
        value: [
          ...
          '`/shadowlock` — Lock silently (no reply message)',
          '`/unlock` — Unlock this channel',
          '`/slowmode <seconds>` — Set slowmode (0 to disable)',
          '`/silence` — Lock every channel in this category',
          '`/unsilence` — Restore every channel in this category',
        ].join('\n'),
      },
      {
      ...
      {
        name: '📋 Mod Records  *(Moderate Members)*',
        value: [
          '`/case <user> <type> <reason>` — Log a manual mod action (warn/mute/ban/kick/note)',
          '`/history <user>` — View full warning & case history for a user',
          '`/watch <user> [reason]` — Flag a user for monitoring',
          '`/unwatch <user>` — Remove monitoring flag',
        ].join('\n'),
      },
      {
        name: '🧹 Cleanup  *(Manage Messages)*',
        value: [
          '`/clear [amount]` — Bulk delete messages (default 10, max 100)',
  ...
  const admin = new EmbedBuilder()
    .setColor(0x2B0040)
    .setDescription('**Admin Commands** *(Manage Guild)*\n\u200b')
    .addFields(
      {
        name: '📢 Announcements',
        value: [
          '`/announce <message>` — Post a server announcement',
          '`/decree <message>` — Issue a royal decree',
          '`/observe <channel>` — Observe a channel\'s activity',
          '`/summon <user>` — Summon a member\'s attention',
        ].join('\n'),
      },
      {
        name: '📊 Polls',
        value: [
          '`/poll create <question> <opt1> <opt2> [opt3–5]` — Start a poll',
          '`/poll results` — Show live vote counts (private)',
          '`/poll end` — Close the poll and announce the winner',
        ].join('\n'),
      },
      {
        name: '🎥 YouTube Notifications',
        value: [
          '`/youtube set` — Subscribe to a channel',
          '`/youtube remove` — Remove a subscription',
          '`/youtube list` — List all subscriptions',
          '`/youtube channel` — Change the notification channel',
          '`/youtube message` — Set a custom notification message *(use `{url}`, `{title}`)*',
          '`/youtube toggle` — Enable or disable a subscription',
          '`/youtube test` — Send a test notification',
        ].join('\n'),
      },
      {
        name: '🎮 Twitch Notifications',
        value: [
          '`/twitch set` — Subscribe to a streamer',
          '`/twitch remove` — Remove a subscription',
          '`/twitch list` — List all subscriptions',
          '`/twitch channel` — Change the notification channel',
          '`/twitch message` — Set a custom message *(use `{url}`, `{game}`, `{viewers}`)*',
          '`/twitch toggle` — Enable or disable a subscription',
          '`/twitch test` — Send a test notification',
        ].join('\n'),
      },
      {
        name: '🎂 Birthday Setup',
        value: [
          ...
          '`/birthday role <role>` — Set the birthday role (assigned for 24h)',
          '`/birthday message set <text>` — Set the server-wide birthday message *(use `{user}`)*',
          '`/birthday message view` — View the current birthday message',
          '`/birthday message remove` — Reset to the default message',
        ].join('\n'),
      },
    )
    .setFooter({ text: 'I am always watching… 👁️' });

  await i.reply({ embeds: [everyone, staff, admin] });
}
...
async function handleBirthday(i) {
  ...
  if (group === 'message') {
    if (!i.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
      return i.reply({ content: '🖤 Only admins can change the server birthday message.', ephemeral: true });
    }
    if (sub === 'set') {
      const text = i.options.getString('text');
      cfg[gid].message = text;
      save('birthday_config.json', cfg);
      return i.reply({ content: `👑🪶 Server birthday message set for all members.\n> ${text}\n\n*Use \`{user}\` to mention the birthday person.*`, ephemeral: true });
    }
    if (sub === 'view') {
      const msg = cfg[gid]?.message;
      if (!msg) return i.reply({ content: '🖤 No custom server birthday message set. Using the default.', ephemeral: true });
      return i.reply({ content: `👑🪶 Current server birthday message:\n> ${msg}`, ephemeral: true });
    }
    if (sub === 'remove') {
      delete cfg[gid].message;
      save('birthday_config.json', cfg);
      return i.reply({ content: '🖤 Server birthday message removed. Reverting to the default.', ephemeral: true });
    }
  }
  ...
  if (sub === 'list') {
    ...
    if (!entries.length) return i.reply({ content: '🖤 No birthdays recorded in this server.' });
  ...
  if (sub === 'channel') {
    ...
    if (!ch) {
      const current = cfg[gid]?.channel;
      return i.reply({
        content: current
          ? `👑🪶 Birthday announcements are currently set to <#${current}>.`
          : '🖤 No birthday channel set yet. Run `/birthday channel <channel>` to set one.',
        ephemeral: true,
      });
    }
  ...
  if (sub === 'role') {
    ...
    const role = i.options.getRole('role');
    if (!role) {
      const current = cfg[gid]?.role;
      return i.reply({
        content: current
          ? `👑🪶 The birthday role is currently set to <@&${current}>.`
          : '🖤 No birthday role set yet. Run `/birthday role <role>` to set one.',
        ephemeral: true,
      });
    }
